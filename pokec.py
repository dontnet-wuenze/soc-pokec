...
def _load_profiles(profile_file):
    ...
    #ts.preaccelerate()
    profiles = pd.read_csv(profile_file, names=names, index_col=False, usecols=names, header=None, sep='\t')

    print("Success load profiles")

    profiles_array = profiles.values
    # translate profiles from slovak to english
    # for i in track(range(0, profiles_array.shape[0])):
    #     for j in track(range(0, profiles_array.shape[1])):
    #         if isinstance(profiles_array[i, j], str):
    #             profiles_array[i, j] = ts.translate_text(profiles_array[i, j], to_language='en', from_language='sk',
    #                                                      translator='Alibaba')
    ...
    profiles_english.to_csv('./process/profiles_english.csv', index=False)
    return profiles
...
def preprocess_data(data_directory='./raw/'):
    ...
    profile_file = os.path.join(data_directory, 'soc-pokec-profiles.txt')
